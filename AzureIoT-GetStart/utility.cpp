...
bool readTMessage(int messageId, char *payload)
{
    ...
    else
    {
        ...
        {
            root["temperatureAlarm"] = 1;
            temperatureAlarm = true;
        }else{
            root["temperatureAlarm"] = 0;
...
bool readHMessage(int messageId, char *payload)
{
    float temperature = readTemperature();
    float humidity = readHumidity();
    StaticJsonBuffer<MESSAGE_MAX_LEN> jsonBuffer;
    ...
    else
    {
        ...
        {
            root["humidityAlarm"] = 1;
            humidityAlarm = true;
        }else{
             root["humidityAlarm"] = 0;
            humidityAlarm = false;
        }
    }
    root.printTo(payload, MESSAGE_MAX_LEN);
